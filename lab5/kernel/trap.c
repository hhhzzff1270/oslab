...
void kerneltrap(struct trapframe *tf) {
    ...
    if (is_intr) {
        ...
        case 2: // Illegal instruction
            printf("illegal instruction at 0x%lx, skip it\n", tf->epc);
            tf->epc += 4;   // 跳过导致异常的那条指令（假设 4 字节指令）
            break;

        case 11: // ecall from M-mode (举例)
            // 处理系统调用...
            tf->epc += 4;   // 同样需要跳过 ecall 指令
            break;

        default:
            printf("trap: unhandled exception %d at epc=0x%lx\n", num, tf->epc);
            while (1) { }  // 或者触发 panic
        }
    }
}